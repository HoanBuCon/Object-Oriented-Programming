...
public class BaiTapTongHop {
	...
	public void hienthiSNT_NhoHon_n() {
		System.out.print("Danh sach so nguyen to nho hon n: ");
		for (int i = 0; i <= n; i++) {
			if (checkSNT(i) == true) {
				System.out.print(i+"\t");
			}
	...
	public void hienthiSNT() {
		System.out.printf("Danh sach %d so nguyen to dau tien: ", n);
		int i = 2, count = 0;
		while (count < n) {
			if (checkSNT(i) == true) {
				System.out.print(i+"\t");
	...
	public void hienthiFIBO() {
		System.out.printf("Danh sach %d so Fibonacci dau tien la: ", n);
		for (int i = 3; i < n; i++) {
